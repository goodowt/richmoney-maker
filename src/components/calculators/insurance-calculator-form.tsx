...
function InsuranceRow({
  ...
  return (
    <div className="grid grid-cols-[1fr_auto_auto] items-baseline gap-4 py-2">
      ...
      <span className="tabular-nums text-foreground/75">{formatWon(employee)}</span>
      <span className="tabular-nums text-foreground/55">{formatWon(employer)}</span>
    </div>
  );
}
    ...
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]">
      ...
      <div className="rounded-2xl border border-border bg-card p-6">
        <div className="mb-4 grid grid-cols-2 gap-4">
          <div>
            <p className="text-sm text-foreground/55">근로자 부담 합계</p>
            <p className="text-2xl font-bold sm:text-3xl">{formatWon(result.employeeTotal)}</p>
          </div>
          <div>
            <p className="text-sm text-foreground/55">사업주 부담 합계</p>
        ...
        <div className="rounded-lg bg-primary-soft/50 px-3 py-2">
          <div className="grid grid-cols-[1fr_auto_auto] gap-4 text-xs font-medium text-foreground/55">
            <span>항목</span>
            <span>근로자</span>
            <span>사업주</span>
          </div>
